...
/*import es.ucm.fdi.tp.basecode.Main.AlgorithmForAIPlayer;
import es.ucm.fdi.tp.basecode.Main.PlayerMode;
import es.ucm.fdi.tp.basecode.Main.ViewInfo;*/
 ...
 * que se quiere jugar y la lista de jugadores.
 */
public class Main {
	 ...
	 * 
	 * <p>
	 * La profundidad maxima del 谩rbol MinMax
	 */
	private static Integer minmaxTreeDepth;
	/**
	 * Atributo para el numero de puerto del server
	 */
	private static Integer serverPort;
	
	/**
	 * Atributo para el host o ip del server
	 */
	...
	private static void parseArgs(String[] args) {
		...
		cmdLineOptions.addOption(constructApplicatioModeOption()); // -am or --app-mode
	...
	/**
	 * Construye la opcion CLI -sh or --srver-host
	 * @return Objeto de esta opcion
	 */
	private static Option constructServerHostOption(){
		
		return new Option("sh", "server-host", true, "El nombre o direccion IP del "
				+ "servidor. Solo se tendr� en cuenta si el application mode es client."
				+ " El valor por defecto es localhost.");
	}
	
	/**
	 * Extrae el valor de server host y lo asigna a su atributo
	 * @param line CLI object
	 * @throws ParseException Salta si no conoce el valor de server host
	 */
	private static void parseServerHostOption(CommandLine line) throws ParseException{
		String hostVal = line.getOptionValue("sh", DEFAULT_SERVERHOST.toString());
		serverHost = hostVal; 
		if(applicationMode != ApplicationMode.CLIENT && serverHost != DEFAULT_SERVERHOST)
			throw new ParseException("Application mode should be Client!");
	}
	
	/**
	 * Construye la opcion CLI -sp or --server-port
	 * @return Objeto de esta opcion
	 */
	private static Option constructServerPortOption(){
		return new Option("sp", "server-port", true, "El puerto del servidor para "
				+ "inicializarlo o para conectar a los clientes. It must has the form: NUM."
				+ " El valor por defecto es 2000");
	}
	
	/**
	 * Extrae el valor de server port y lo asigna a su atributo
	 * @param line CLI object
	 * @throws ParseException Salta si no conoce el valor de server port
	 */
	...
	/**
	 * Construye la opcion CLI -am or --app-mode
	 * @return Objeto de esta opcion
	 */
	private static Option constructApplicatioModeOption(){
		String optionInfo = "The application mode to play ( ";
		...
		opt.setArgName("application mode identifier");
		return opt;
	}
	
	/**
	 * Extrae el valor de application mode y lo asigna a su atributo
	 * @param line CLI object
	 * @throws ParseException Salta si no conoce el valor de appication mode
	 */
	private static void parseApplicatioModeOption(CommandLine line) throws ParseException{
		String appModeVal = line.getOptionValue("am", DEFAULT_APPMODE.getId());
		
		for (ApplicationMode a : ApplicationMode.values()) {
			if (appModeVal.equals(a.getId())) {
				applicationMode = a;
			}
		}
	 ...
	 * @return opcion de crear obstaculo
	 */
	private static Option constructObstaclesOption() {
		return new Option("o", "obstacle", true,
				"The numbre of obstacle . The position is random.");
	}
	
	/**
	 *  Extrae la opcion obstacles (-o).
	 * @param line
	 *            CLI {@link CommandLine} object.
	 * @throws ParseException  
	 *          Si se proporciona un valor invalido 
	 */
	private static void parserObstaclesOptions(CommandLine line)throws ParseException {
		String obsVal = line.getOptionValue("o");
		if (obsVal != null) {
			try {
				String[] dim = obsVal.split("x");
				if (dim.length == 1) {
					obstacles = Integer.parseInt(dim[0]);
				} else {
		...
		case Ataxx:
			if(dimRows != null && dimCols != null){
				...
				if(obstacles!=null)
					...
					gameFactory = new AtaxxFactoryExt(0);
			}break;
		default:
	 ...
	 * 
	 * <p>
	 * M茅todo para iniciar un juego con el controlador {@link ConsoleCtrl}, no
	...
	/**
	 * Inicia un cliente del servidor o jugador
	 */
	private static void startClient() {
		try{
			...
			System.err.println("We cant conect");
		}
	}
	 ...
	 * un juego con {@link #startGame()}, inicia un servidor con {@link #startServer()} 
	 * o inicia un cliete con {@link #startClient()}.
	 * 
